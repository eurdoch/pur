...
@group(0) @binding(4) var<storage, read> padded_input: array<f32>;
...
fn backward_weights(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    for (var oh = 0u; oh < output_height; oh = oh + 1u) {
        for (var ow = 0u; ow < output_width; ow = ow + 1u) {
            ...
            let input_val = padded_input[get_padded_input_idx(0u, ic, h_start + kh, w_start + kw)];
...
fn backward_input(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let oh_start = max(0u, (ih as i32 + params.padding as i32 - params.kernel_height as i32 + params.stride as i32) / params.stride as i32);
    let oh_end = min(output_height, (ih + params.padding) / params.stride + 1u);
    let ow_start = max(0u, (iw as i32 + params.padding as i32 - params.kernel_width as i32 + params.stride as i32) / params.stride as i32);
    ...
    padded_input[input_idx] = grad_sum;
}
